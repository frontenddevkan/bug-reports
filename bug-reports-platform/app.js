...
const CHECKLISTS = {
    ...
    'fruto-registration': {
        id: 'fruto-registration',
        title: 'Регистрация: frutonyanya.ru',
        items: [
            'Страница регистрации доступна с главной страницы (кнопка/ссылка «Войти» → «Регистрация» или аналогичный путь).',
            'Форма регистрации отображается полностью: видны все поля, подписи, чекбоксы согласий и кнопка отправки.',
            'Регистрация с валидным набором данных (уникальный email/телефон, корректный пароль, заполнены обязательные поля) проходит успешно.',
            'При попытке зарегистрировать уже существующий email/телефон отображается понятное сообщение об ошибке («пользователь уже существует» или аналог).',
            'Поля email/телефона валидируются: некорректный формат (без @, неполный номер) не принимается, выводится сообщение об ошибке.',
            'Пароль и его подтверждение проверяются на совпадение и минимальные требования (длина, состав), при нарушении — подсветка и текст ошибки.',
            'При отправке пустой формы подсвечиваются все обязательные поля, пользователь видит, что именно нужно заполнить.',
            'Если есть e‑mail‑подтверждение, после регистрации на почту приходит письмо с корректным текстом и рабочей ссылкой активации.',
            'После ошибки в одном поле остальные введённые данные не теряются (форму не нужно заполнять заново).',
            'Тексты подсказок и сообщений об ошибках понятны пользователю (без технического жаргона), соответствуют языку сайта.'
        ]
    }
};
...
function initBgChargeCanvas() {
    ...
    function edgeColor(x1, y1, x2, y2) {
        ...
        const a = Math.min(1, 0.05 * fade);
        return `rgba(15,50,80,${a})`;
    }

    // Строим граф рёбер шестиугольной сетки
        ...
        for (let row = -1; row < rows; row++) {
            for (let col = -1; col < cols; col++) {
                ...
                for (let i = 0; i < 6; i++) {
                    ...
                    const k1 = vKey(v1.x, v1.y);
                    const k2 = vKey(v2.x, v2.y);
                    addAdj(k1, { x: v2.x, y: v2.y });
                    addAdj(k2, { x: v1.x, y: v1.y });
                }
            }
        }
        allEdges = edges;
    ...
    // Создаём маршруты для огоньков
    function buildDotRoutes() {
        const routes = [];
        ...
        for (let i = 0; i < NUM_DOTS; i++) {
            const key = keys[(step * (i + 1)) % keys.length];
    ...
    function redrawGrid() {
        ...
        // Тень для объёма — едва заметные
        gridCtx.save();
        hexCenters.forEach(c => {
            const fade = fadeFactor(c.x, c.y);
            const shadowA = Math.min(1, 0.04 * fade);
        ...
        // Glass-заливка: почти невидимые
        hexCenters.forEach(c => {
            const fade = fadeFactor(c.x, c.y);
            const g = gridCtx.createLinearGradient(c.x, c.y - GLASS_RADIUS, c.x, c.y + GLASS_RADIUS);
            const a1 = Math.min(1, 0.012 * fade);
            const a2 = Math.min(1, 0.006 * fade);
            const a3 = Math.min(1, 0.002 * fade);
            g.addColorStop(0, `rgba(150,190,240,${a1})`);
            g.addColorStop(0.3, `rgba(130,170,220,${a2})`);
            g.addColorStop(1, `rgba(110,150,200,${a3})`);
            gridCtx.fillStyle = g;
            gridCtx.beginPath();
        ...
        // Блик на верхней грани — едва заметный
        hexCenters.forEach(c => {
            const fade = fadeFactor(c.x, c.y);
            const highlightA = Math.min(1, 0.018 * fade);
        ...
        allEdges.forEach(e => {
            gridCtx.strokeStyle = edgeColor(e.x1, e.y1, e.x2, e.y2);
            gridCtx.beginPath();
            gridCtx.moveTo(e.x1, e.y1);
            gridCtx.lineTo(e.x2, e.y2);
            gridCtx.stroke();
        });
    }
    ...
    function drawDot(x, y, intensity) {
        const coreR = 1.5;
        const glowR = 8 + 3 * intensity;

        const g = ctx.createRadialGradient(x, y, 0, x, y, glowR);
        g.addColorStop(0, `rgba(150,200,250,${0.20 + 0.25 * intensity})`);
        g.addColorStop(0.25, `rgba(80,160,240,${0.08 + 0.14 * intensity})`);
        g.addColorStop(0.55, `rgba(30,100,200,${0.03 + 0.08 * intensity})`);
        g.addColorStop(1, 'rgba(15,50,120,0)');
        ...
        ctx.fillStyle = `rgba(170,210,250,${0.18 + 0.30 * intensity})`;
        ctx.beginPath();
    ...
    function flashIntensity(tSec, delaySec) {
        const local = ((tSec - delaySec) % FLASH_PERIOD + FLASH_PERIOD) % FLASH_PERIOD;
        if (local > 1) return 0;
    ...
    function dotPosition(route, tSec, speed, delay) {
        ...
        const segProgress = (t - segIdx * segTime) / segTime;
    ...
    // Находим ближайшие hex-центры к точке (до заданного радиуса)
    function findNearbyHexCenters(px, py, radius) {
        const result = [];
        for (let i = 0; i < hexCenters.length; i++) {
            ...
            if (dist < radius) {
                result.push({ x: c.x, y: c.y, dist });
            }
        }
        return result;
    }

    // Рисуем подсветку ячейки: мягкое свечение всех 6 граней + bloom
    function drawHexGlow(cx, cy, brightness) {
        if (brightness < 0.005) return;
        const r = HEX_RADIUS;

        // Мягкое bloom-свечение вокруг
        const bloomR = r * 2.2;
        const bloom = ctx.createRadialGradient(cx, cy, r * 0.3, cx, cy, bloomR);
        bloom.addColorStop(0, `rgba(40,130,220,${0.07 * brightness})`);
        bloom.addColorStop(0.5, `rgba(25,90,180,${0.03 * brightness})`);
        bloom.addColorStop(1, 'rgba(15,50,120,0)');
        ctx.fillStyle = bloom;
        ctx.beginPath();
        ctx.arc(cx, cy, bloomR, 0, Math.PI * 2);
        ctx.fill();

        // Лёгкая заливка ячейки
        const g = ctx.createLinearGradient(cx, cy - GLASS_RADIUS, cx, cy + GLASS_RADIUS);
        g.addColorStop(0, `rgba(30,100,200,${0.04 * brightness})`);
        g.addColorStop(1, `rgba(15,60,140,${0.01 * brightness})`);
        ctx.fillStyle = g;
        ctx.beginPath();
        hexPath(ctx, cx, cy, GLASS_RADIUS);
        ctx.fill();

        // Все 6 граней — 3 слоя для неонового эффекта
        // Широкий glow
        ctx.strokeStyle = `rgba(30,120,220,${0.20 * brightness})`;
        ctx.lineWidth = 3.5;
        ctx.lineJoin = 'round';
        ctx.lineCap = 'round';
        ctx.beginPath();
        ...
        // Средний
        ctx.strokeStyle = `rgba(60,160,240,${0.30 * brightness})`;
        ctx.lineWidth = 1.8;
        ctx.beginPath();
        hexPath(ctx, cx, cy, r);
        ctx.stroke();
        ...
        // Яркое ядро
        ctx.strokeStyle = `rgba(130,210,255,${0.45 * brightness})`;
        ctx.lineWidth = 0.8;
        ctx.beginPath();
        hexPath(ctx, cx, cy, r);
        ctx.stroke();
    ...
    // Плавное свечение: персистентная карта яркости hex-ов
    // Каждый hex плавно нарастает/убывает (lerp к целевому значению)
    const glowMap = new Map(); // key -> { current: 0, target: 0 }
    const GLOW_RISE = 0.03;   // скорость нарастания за кадр
    const GLOW_FADE = 0.012;  // скорость затухания за кадр
    // Радиус распространения свечения на соседние гексы
    const SPREAD_RADIUS = HEX_RADIUS * 3.5; // ~2 кольца гексов
    ...
    function render(tSec) {
        ...
        // Сбрасываем все target'ы в 0
        glowMap.forEach(v => { v.target = 0; });

        // Огоньки движутся по рёбрам: задаём target-яркость с распространением
        for (let i = 0; i < dotRoutes.length; i++) {
            ...
            if (pos.ex1 !== undefined) {
                const mx = (pos.ex1 + pos.ex2) / 2;
                const my = (pos.ey1 + pos.ey2) / 2;

                // Находим гексы в радиусе распространения
                const nearby = findNearbyHexCenters(mx, my, SPREAD_RADIUS);
                nearby.forEach(c => {
                    const key = `${c.x},${c.y}`;
                    if (!glowMap.has(key)) glowMap.set(key, { current: 0, target: 0 });
                    const entry = glowMap.get(key);
                    // Яркость убывает с расстоянием
                    const distFactor = 1 - c.dist / SPREAD_RADIUS;
                    const baseBright = 0.25 + 0.15 * intensity;
                    const contrib = baseBright * distFactor * distFactor; // квадратичное затухание
                    entry.target = Math.min(1, entry.target + contrib);
                });
            }
        }

        // Плавно интерполируем current → target
        glowMap.forEach((v, key) => {
            if (v.current < v.target) {
                v.current = Math.min(v.target, v.current + GLOW_RISE);
            } else {
                v.current = Math.max(v.target, v.current - GLOW_FADE);
            }
            ...
            if (v.current > 0.005) {
                ...
                drawHexGlow(cx, cy, v.current);
            }
        });

        // Рисуем сами огоньки и подсветку текущего ребра
        for (let i = 0; i < dotRoutes.length; i++) {
            const route = dotRoutes[i];
            const speed = 1.0 + (i % 4) * 0.25;
            const delay = i * 3.2;
            ...
            if (pos.ex1 !== undefined) {
                ctx.strokeStyle = `rgba(80,170,240,${0.10 + 0.18 * intensity})`;
                ctx.lineWidth = 1.2;
                ctx.beginPath();
...
const QUIZ_QUESTIONS = [
    ...
    {
        q: 'Чем различаются позитивное и негативное тестирование?',
        options: [
            { t: 'Позитивное проверяет корректную работу при ожидаемых данных, негативное — поведение при некорректном вводе и ошибочных условиях', ok: true, why: 'Это два комплементарных подхода: сначала убеждаемся, что “нормальное” работает, затем ищем, где и как система ломается на ошибках.' },
            { t: 'Позитивное — это всегда ручное тестирование, а негативное — всегда автоматизированное', ok: false, why: 'Позитивные и негативные сценарии могут быть как ручными, так и автоматизированными.' },
            { t: 'Позитивное тестирование нужно только на старте проекта, а негативное — только перед релизом', ok: false, why: 'Оба типа тестов полезны на всех этапах разработки и регресса.' },
        ],
    },
    {
        q: 'Что относится к статическому, а что — к динамическому тестированию?',
        options: [
            { t: 'Статическое — анализ требований и кода без запуска программы; динамическое — выполнение тестов на работающем приложении', ok: true, why: 'Ревью требований, документации и кода — статическое тестирование, запуск тест‑кейсов на билде — динамическое.' },
            { t: 'Статическое — это только ручное тестирование UI, динамическое — только автотесты API', ok: false, why: 'UI‑проверки — динамика, а статический анализ бывает и автоматизированным (линтеры, сканеры кода).' },
            { t: 'Статическое тестирование проводится только в продакшене, динамическое — только на dev', ok: false, why: 'Место окружения не определяет “статичность”; важен сам подход — с запуском кода или без.' },
        ],
    },
    {
        q: 'В чём разница между ручным и автоматизированным тестированием?',
        options: [
            { t: 'При ручном тестировании шаги выполняет человек, при автоматизированном — сценарии выполняются инструментами/скриптами', ok: true, why: 'Ручное тестирование гибко и хорошо для исследований, авто‑тесты удобны для частых повторов и регресса.' },
            { t: 'Ручное тестирование всегда лучше автоматизированного и полностью его заменяет', ok: false, why: 'Обычно используют сочетание: часть регресса автоматизируют, исследовательские проверки остаются ручными.' },
            { t: 'Автоматизированное тестирование не требует участия QA и всегда находит больше багов', ok: false, why: 'Автотесты находят ровно те баги, против которых их написали; дизайн тестов и анализ результатов по‑прежнему делают люди.' },
        ],
    },
    {
    ...
    {
        q: 'По какому правилу в отчёте ставится признак «Победитель с не наименьшей ценой»?',
        options: [
            {
                t: 'Если среди всех заявок (на основном этапе и на переторговке) есть цена строго ниже цены победителя',
                ok: true,
                why: 'Признак = «Да», только если победитель выбран при наличии более дешёвой заявки.',
            },
            {
                t: 'Если победитель подал заявку на переторговке, а не на основном этапе',
                ok: false,
                why: 'Этап подачи заявки не определяет, является ли цена минимальной.',
            },
            {
                t: 'Если цена победителя равна минимальной цене среди всех заявок',
                ok: false,
                why: 'В этом случае признак должен быть «Нет» — победитель с минимальной ценой.',
            },
        ],
    },
    {
    ...
    },
    {
        ...
        options: [
            {
                t: 'Документ с подробной последовательностью шагов, входными данными и ожидаемыми результатами для проверки конкретной функциональности',
                ok: true,
                why: 'Тест‑кейс — это детальная инструкция “как тестировать” и что ожидать на каждом шаге.',
            },
            {
                t: 'Любой список проверок без указания шагов и ожидаемого результата',
                ok: false,
                why: 'Это больше похоже на чек‑лист, а не на полноценный тест‑кейс.',
            },
            {
                t: 'Случайный набор баг‑репортов по фиче',
                ok: false,
                why: 'Баг‑репорты описывают найденные дефекты, а не план проверки.',
            },
        ],
    },
    {
    ...
    {
        q: 'Что такое взаимный просмотр (рецензирование) требований?',
        options: [
            {
                t: 'Обсуждение требований коллегами (аналитиками, разработчиками, тестировщиками, дизайнерами) с целью найти ошибки, противоречия и улучшить формулировки',
                ok: true,
                why: 'Взаимный просмотр — одна из основных техник тестирования требований через совместное рецензирование документа.',
            },
            {
                t: 'Автоматический прогон автотестов по требованиям',
                ok: false,
                why: 'Автотесты проверяют реализованный продукт, а не текст требований.',
            },
            {
                t: 'Одностороннее чтение требований только автором без обратной связи',
                ok: false,
                why: 'Без обратной связи от команды сложно заметить неочевидные ошибки и противоречия.',
            },
        ],
    },
    {
        q: 'Как с помощью тест‑кейсов и чек‑листов проверить проверяемость требований?',
        options: [
            {
                t: 'Если по каждому требованию можно быстро придумать пункты чек‑листа или тест‑кейсы с понятным ожидаемым результатом',
                ok: true,
                why: 'Когда по требованию легко формулируются проверки, оно, как правило, достаточно конкретно и проверяемо.',
            },
            {
                t: 'Если требования сформулированы максимально общими словами без конкретных сценариев',
                ok: false,
                why: 'Размытые формулировки затрудняют составление тестов и делают требования непроверяемыми.',
            },
            {
                t: 'Если для требований совсем не нужно придумывать тесты',
                ok: false,
                why: 'Отсутствие тестов говорит о том, что проверить выполнение требований невозможно или о них забыли.',
            },
        ],
    },
    {
    ...
    {
        q: 'В чём разница между тестированием чёрного и белого ящика?',
        options: [
            {
                t: 'Чёрный ящик проверяет только вход/выход без знания кода, белый ящик требует знания внутренней логики и кода',
                ok: true,
                why: 'Black-box тестирует поведение системы, white-box — внутреннюю реализацию и структуру кода.',
            },
            {
                t: 'Чёрный ящик используют только разработчики, белый ящик — только тестировщики',
                ok: false,
                why: 'Наоборот: разработчики чаще используют white-box (unit-тесты), тестировщики — black-box.',
            },
            {
                t: 'Это одно и то же, просто разные названия',
                ok: false,
                why: 'Это принципиально разные подходы к тестированию с разной глубиной погружения в код.',
            },
        ],
    },
    {
        q: 'Что такое таблица переходов (State Transition Testing)?',
        options: [
            {
                t: 'Метод тестирования динамического поведения через проверку переходов между состояниями системы',
                ok: true,
                why: 'State Transition проверяет как система переходит из одного состояния в другое при различных событиях.',
            },
            {
                t: 'Таблица для тестирования статических страниц сайта',
                ok: false,
                why: 'Таблица переходов относится к динамическому поведению, а не к статическому контенту.',
            },
            {
                t: 'Список всех возможных багов в системе',
                ok: false,
                why: 'Это не про баги, а про состояния и переходы между ними — нормальное поведение системы.',
            },
        ],
    },
];
